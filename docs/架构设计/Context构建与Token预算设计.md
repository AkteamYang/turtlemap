...
第一阶段直接采用稳定模板，便于后续按章节做增量压缩、合并和重写。
...
因此章节集合保持稳定，不让模型在不同轮次随意新增和删除章节。

##### 原则四：默认优先合并、替换、删除，最后才是新增

当新信息进入 `memory` 时，优先顺序应为：
...
当前阶段直接由 `Tokenizer` 持有一个 `TokenCache` 属性，`TokenCache`
使用 `OrderedDict` 实现最小 LRU 能力，不额外引入缓存依赖。

这样设计的原因是：

- 当前使用“对象级缓存”，而不是函数装饰器式缓存
- 需要由 `Tokenizer` 自己持有缓存状态，便于后续做命中统计、手动清理和按实例隔离
- 当前主要诉求是限制总缓存条目数，`OrderedDict` 足够简单直接

这里不优先直接使用标准库 `functools.lru_cache`，因为它适合纯函数级记忆化，
不太适合作为 `Tokenizer` 内部一个可显式管理的缓存容器。

第一阶段把缓存容量定义为“最大缓存条目数”，而不是“最大内存字节数”。
...
后续如果要实现统一压缩策略，固定压缩优先级与迭代流程，避免每轮行为漂移。
